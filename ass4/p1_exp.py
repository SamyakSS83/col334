        ...
        for DELAY in delay_list:
            for JITTER in jitter_list:
                for i in range(0, NUM_ITERATIONS):
                    # remove previous received file before running this iteration
                    try:
                        if os.path.exists(OUTFILE):
                            os.remove(OUTFILE)
                            print(f"Removed existing file: {OUTFILE}")
                    except Exception as e:
                        print(f"Warning: could not remove {OUTFILE}: {e}")
                    ...
                    h1.cmd(f"python3 p1_server.py {SERVER_IP} {SERVER_PORT} {SWS} > server_{i}_{LOSS}_{DELAY}_{JITTER}.log 2>&1 &")
                    time.sleep(1)
