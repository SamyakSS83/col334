...
# Note: Mininet does not set link weights; controller will use config.json weight matrix
class AsymmetricCosts(Topo):
    def build(self):
...
# topos = { 'asymcost': AsymmetricCosts }
def run():
    """Create the network, start it, and enter the CLI."""
    topo = AsymmetricCosts()
